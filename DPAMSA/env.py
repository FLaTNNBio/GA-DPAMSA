...
class Environment:
    ...
    def set_alignment(self, seqs):
        if isinstance(seqs[0][0], int):  # Se i dati sono già numerici
            self.aligned = seqs
        else:  # Se i dati sono in formato stringa, convertili
    ...
    def get_alignment(self):
        ...
        for seq in self.aligned:
            if isinstance(seq[0], int):  # Se sono numeri, converti in lettere
                alignment += ''.join([nucleotides[n - 1] for n in seq]) + '\n'
            else:  # Se sono già lettere, unisci direttamente
                alignment += ''.join(seq) + '\n'
