...
The variable ```conserved_block_sizes``` set the number and lengths of conserved blocks to prevent complete random dataset and archive at the list the ```min_score_threshold``` to create a controlled and stable environment for generating synthetic datasets designed to emulate real biological conditions while maintaining consistency for comparative analysis.
